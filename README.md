# Xây dựng hệ thống ngăn chặn tấn công Brute-force bằng Fail2ban kết hợp giám sát tập trung bằng Zabbix
...
*   **Zabbix Server (CentOS/Docker)**: Máy chủ quản lý giám sát và Dashboard.
*   **Attacker (Kali Linux)**: Sử dụng công cụ Hydra để thực hiện tấn công mô phỏng.

![Chèn sơ đồ kiến trúc hệ thống tại đây](/img/theme.png)
...
### 2. Tấn công mô phỏng (Kali Linux)
...
```
![tấn công](img/tancong.png)

### 3. Ngăn chặn tự động với Fail2ban (IPS)
...
### 4. Giám sát & Cảnh báo với Zabbix (Monitoring)
...
## 1. Xây dựng môi trường ứng dụng (Docker)
Để giả lập môi trường thực tế, ứng dụng Web được triển khai trong Container và được cấu hình để lưu trữ log ra máy Host.

*   **Bước 1.1: Viết mã nguồn ứng dụng (PHP)**
    ...
    > **![Ảnh 11](/img/web.png)** Mã nguồn PHP xử lý xác thực và in ra thông báo lỗi.
...
## 2. Cấu hình Ngăn chặn tự động (Fail2ban)
Fail2ban đóng vai trò là lớp bảo vệ trực tiếp, phân tích log và thực thi lệnh chặn.

*   **Bước 2.1: Tạo bộ lọc (Filter)**
    Định nghĩa biểu thức chính quy (Regex) trong file `my-docker-app.conf` để nhận diện các dòng log đăng nhập lỗi từ Docker.
    > **![Ảnh 7](/img/Regex.png)** Cấu hình Regex bắt các request "POST" trả về mã "200" (xác thực sai).

*   **Bước 2.2: Thiết lập quy tắc chặn (Jail)**
    Cấu hình file `jail.local` để chỉ định mục tiêu bảo vệ, ngưỡng chặn (maxretry) và hành động chặn thông qua chain `DOCKER-USER`.
    > **![Ảnh 12](/img/jail.local.png)** Nội dung file cấu hình `jail.local`.

---

## 3. Cấu hình Giám sát (Zabbix Agent 2)
Cài đặt Zabbix Agent trên Web Server để gửi dữ liệu về Zabbix Server.

*   **Bước 3.1: Cấu hình Agent Active**
...
*   **Bước 4.4: Thiết lập cảnh báo (Trigger)**
    ...
    > Thao tác menu để tạo Trigger.**![Bước 1](/img/trigger1.png)
    ![Bước 2](/img/trigger2.png)** 
    > Cấu hình chi tiết Trigger: Tên, Expression (count) và Recovery expression (nodata).![Bước 3](/img/trigger3.png)

---

## 5. Tấn công mô phỏng và Kiểm chứng kết quả
Sử dụng Kali Linux làm môi trường tấn công để kiểm tra sức mạnh của hệ thống.

*   **Bước 5.1: Thực hiện tấn công Hydra**
    Dùng Hydra bắn liên tục các yêu cầu đăng nhập vào Web Server.
    > **![Ảnh 17](/img/tancong.png)** Màn hình Kali Linux: Hydra bắt đầu tấn công và sau đó bị báo lỗi "Connection errors" (Do đã bị chặn).

*   **Bước 5.2: Kiểm tra log hệ thống**
    Quan sát file log của Web Server ghi nhận các request dồn dập và log của Fail2ban thực hiện lệnh Ban.
    > **![Ảnh 15](/img/logweb.png)** Log Web Server bị tràn ngập bởi request từ Hydra.
    > **![Ảnh 14](/img/logfail2ban.png)** Log Fail2ban ghi nhận hành vi và thực hiện lệnh "Ban".

*   **Bước 5.3: Xác nhận trạng thái chặn**
    Sử dụng lệnh kiểm tra trên CentOS để thấy IP máy tấn công đã nằm trong danh sách đen.
    > **![Ảnh 13](/img/listbanIP.png)** Lệnh `fail2ban-client status` hiển thị IP 192.168.25.151 đang bị Banned.

*   **Bước 5.4: Dashboard Cảnh báo thời gian thực**
    Màn hình Zabbix hiện cảnh báo đỏ rực rỡ, chỉ đích danh IP đang tấn công.
    > **![Ảnh 21](/img/canhbao.png)** Dashboard Zabbix hiển thị cảnh báo: "Fail2ban đã chặn IP: 192.168.25.151".
