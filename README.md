 ...
 **![Ảnh 9](img/Dockerfile.png)** Nội dung Dockerfile copy mã nguồn vào thư mục `/var/www/html/`.
...
*   **Bước 1: Tạo Dashboard SOC**
    ...
    > ![Ảnh chụp toàn cảnh Dashboard Splunk](img/splunk-dashboard-overview.png)
...
*   **Bước 2: Vẽ biểu đồ "Top IP Tấn Công"**
    ...
    | sort - count
    ```
    > ![Ảnh chụp biểu đồ cột Top IP Tấn Công](img/splunk-top-attackers-chart.png)
...
*   **Bước 3: Tạo bảng "Trạng thái IP Bị Chặn" (Ban/Unban)**
    ...
    ```
    > ![Ảnh chụp bảng trạng thái Ban/Unban](img/splunk-ban-status-table.png)
...
*   **Bước 4: Cấu hình Alert tự động**
    ...
    * ``eval Status = case(...)``: Áp dụng logic điều kiện để tự động gán nhãn trạng thái "Mới" (kèm icon 🔥) hoặc "Cũ" (kèm icon 💤) dựa trên việc sự kiện đó có xảy ra trong vòng 2 phút (120 giây) gần nhất hay không.
    ...
    ```
    > ![Ảnh chụp danh sách cảnh báo đã được kích hoạt trong mục Activity](img/splunk-triggered-alerts.png)
